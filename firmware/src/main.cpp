 ...
 *   k<0/1>    首颗启动缓冲（不吃灯珠）              （会永久保存）
 *   g<0/1>    第一颗红绿对调                        （会永久保存）
 *   z<数字>   只点亮第 N 颗（定位用，不保存）
 *   a<RRGGBB> 改"单颗定位"的颜色（不保存）
 *   t         自检：红 → 绿 → 蓝 → 白
 *   w         看 Wi-Fi 热点信息
 *   ?         打印帮助（含当前全部设置）—— 这条永远最准
 *
 *   0~7 是快捷键，注意编号和灯效编号差一位（都会永久保存）：
 *   1=彩虹流动  2=逐颗扫描  3=整条呼吸  4=彗星来回
 *   5=全白长亮  6=整体变色  7=纯色常亮  0=关灯
 *
 *   排查闪烁用（都不保存，断电回默认）：
 *   p<0~3>    数据波形档      x0/x1  老 RMT 分包 / 整帧直发
 *   y<kHz>    数据速度，默认 2500
 *   h<0~64>   SPI 起跑缓冲字节数
 *   r<1~5>    同一帧连发几遍
 *
 *   完整对照表见仓库 references/board-protocol.md
 */
#include <Arduino.h>
